...
=== 5.2. Write optimized Dockerfiles

In Dockerfiles, you can use two techniques to optimize your build process. You can combine multiple `RUN`s so that the initial packages installations and the final deletions (of compilers, development libraries and caches/temporary files) are left within the same layer. This approach has the disadvantage that the Dokerfile (recipe) is difficult to read and interpret.

In order to a more readable approach you can name your build stages. By default, the stages are not named, and you refer to them by their integer number, starting with 0 for the first FROM instruction. However, you can name your stages, by adding an as _<NAME>_ to the FROM instruction.
